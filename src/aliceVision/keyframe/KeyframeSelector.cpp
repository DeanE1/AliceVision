    ...
    , _outputFolder(outputFolder)
{
    ...
    if (mediaPaths.empty()) {
        ALICEVISION_LOG_ERROR("Cannot create KeyframeSelector without at least one media file path!");
        throw std::invalid_argument("Cannot create KeyframeSelector without at least one media file path!");
    }
}
                                      ...
                                      const std::vector<float>& mmFocals)
{
    ...
    for (int id = 0; id < _mediaPaths.size(); id++) {
        ...
        for (auto pos : _selectedKeyframes) {
